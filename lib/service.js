 ...
 */
/*Service.prototype.fetchTaskDefinition = function(taskDefinition) {
  var s = this;
  return new Promise(function(resolve, reject) {
    s.ecs.describeTaskDefinition({ taskDefinition: taskDefinition.family }, function(err, response) {
      if (err || !response.taskDefinition) {
        reject(err);
        return;
      }

      s._logProgress('Task definition retrieved');

      // We only return parts of the task def response.
      resolve({
...
Service.prototype.registerTaskDefinition = function(version) {
  var s = this;
  return new Promise(function(resolve, reject) {
    s.ecs.registerTaskDefinition(s.service.taskDefinition, function(err, response) {
      if (err) {
        reject(err);
        return;
      }
      s._logProgress('Registered task def in ECS:' + response.taskDefinition.taskDefinitionArn);
  ...
  return new Promise(function(resolve, reject) {
    s.fetchAutoScalingGroup().then(function(asg) {
      ...
      if (asg.DesiredCapacity === 0) {
        s._logProgress('Skipping auto-scale scale up since desired capacity is 0');
        resolve();
        return;
      }
